...
public class GTRServerSave {
    ...
    public static String getSaveFilePath(String packageName, long startTestTime, int pid){
        ...
        String dataFilePath = PrismConfig.gtrDirPath+packageName+"_"+dateStr+"/"+pid+".txt";
        return dataFilePath;
    }

    public static String getSaveFileDir(){
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy.MM.dd_HH:mm:ss");
        Date curDate = new Date(GTRCollector.getStartTime());//获取当前时间
        String dateStr = dateFormat.format(curDate);
        String dataFilePath = PrismConfig.gtrDirPath+GTRCollector.applicationContext.getPackageName()+"_"+dateStr;
        return dataFilePath;
    }
