...
function getChartDataInArea_userTag(startTime, endTime){			//图-区间操作区间数据
	var array = new Array();
	for(var i=0;i<tagInfo.length;i++){
		var page = new Array();
		var start = tagInfo[i].start;
		var end = tagInfo[i].end;
		start = start - startTime;
		end = end - startTime;
		page.push({xAxis: start});
		page.push({xAxis: end});
...
function getToolTip_UserTag(time){
	time = time + appInfo.startTestTime;
	var tip = "";
	for(var i=0;i<tagInfo.length;i++){
		if(tagInfo[i].start<=time && time<=tagInfo[i].end){
			tip = tip+"用户标识："+ tagInfo[i].className + "." + tagInfo[i].methodName +" -- "+(tagInfo[i].end-tagInfo[i].start)+"ms <br />";
		}
	}
	return tip;
}

function getToolTip_SingleUserTag(time,startTime,endTime){
	time = time + appInfo.startTestTime;
	var tip = "";
	for(var i=0;i<tagInfo.length;i++){
	    if (tagInfo[i].start==startTime && endTime==tagInfo[i].end) {
		    if(tagInfo[i].start<=time && time<=tagInfo[i].end){
			    tip = tip+"用户标识："+ tagInfo[i].className + "." + tagInfo[i].methodName +" -- "+(tagInfo[i].end-tagInfo[i].start)+"ms <br />";
		        return tip;
		    }
...
function getTableData_baseChartData(){	
	...
	for(var i=0;i<normalInfos.length;i++){
		if(tempLastCpuApp!=-1){
		    ...
		    //prism 由于多个场景启动使app重启 
		    //导致cpu memory flow会有多个负值区
		    if (cpu < 0) {
		        cpu = 0;
		    }
...
function getChartDataInArea_forBlock(startTime,endTime){
	...
	chartDataInArea_forBlock.userTagData = getChartDataInArea_userTag(startTime, endTime);
...
function getChartDataInArea_forPage(pageLoadInfo){
	...
	chartDataInArea_forPage.userTagData = getChartDataInArea_userTag(startTime,endTime);
...
/////////// 用户标识页-表格数据：//////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
function getTableData_userTag(){
	var tableData_userTag = new Array();
	for(var i = 0; i <tagInfo.length; i++){
	    var pageLoadInfo = getPageLoadInfoByTime(tagInfo[i].start,tagInfo[i].end);
		var thisData = new Object();
		thisData.dataID = i;
		thisData.methodName = tagInfo[i].methodName;
		thisData.className = tagInfo[i].className;
		thisData.spendTime = tagInfo[i].end-tagInfo[i].start;
		thisData.start = tagInfo[i].start;
		thisData.end = tagInfo[i].end;
		thisData.pageLoadInfo = pageLoadInfo;
		tableData_userTag.push(thisData);
	}

	return tableData_userTag;
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////// 内存泄漏页-表格数据：//////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
function getTableData_leak(){
	var tableData_leak = new Array();
	for(var i = 0; i <leakInfo.length; i++){
		var thisData = new Object();
		thisData.className = leakInfo[i].className;
		thisData.time = getRealTime(leakInfo[i].time);
		tableData_leak.push(thisData);
	}

	return tableData_leak;
}
...
function getChartDataInArea_forGC(gcInfo){
	...
	chartDataInArea_forGC.userTagData = getChartDataInArea_userTag(startTime,endTime);
...
function openPage(index){
	...
	//用户标识页
    	...
    	document.getElementById("page_tag").style.display="block";	//显示Html
    	show_userTagTable();
    }
    //内存泄漏页
    else if(index==7){
    	document.getElementById("page_leak").style.display="block";	//显示Html
    	show_leakTable();
    }
	//G C检测页
	else if(index==8){
...
function removeAllPage(){
	...
	document.getElementById("page_tag").style.display="none";
	document.getElementById("page_leak").style.display="none";
...
function showTestResultSummaryTable(){
	...
	if(tableBaseData_explicitGC==null || tableBaseData_explicitGC.length==0){
		$('#result_memory').html('<p style="color:green">未发现显式GC调用！</p>');
	}else{
		$('#result_memory').html('<p style="color:#EE4000">发现'+tableBaseData_explicitGC.length+'个式GC调用！</p>');
	}
...
function show_LowSMTableDetail(data){
	...
	var areaChartOption = {
		...
		legend: {
			data:['流畅值（帧/s）','卡顿时长（ms）','View构建','操作','用户标识'],
			x: 'left'
		},
		xAxis : [
			{
		...
		tooltip: {
			...
			formatter: function(params){
				...
				//tip = tip + getToolTip_Draw(pageLoadInfo,xAxisValue);
				tip = tip + getToolTip_ViewBuild(xAxisValue);
				tip = tip + getToolTip_UserTag(xAxisValue);
				//tip = tip + getToolTip_Lifecycle(pageLoadInfo,xAxisValue);
				return tip;
			}
		}
...
function show_pageLoadDetail(fatherTable,pageLoadInfo){
	...
	var chart_option= {
		...
		legend: {
			data:['Activity生命周期函数','卡顿数据','流畅值（帧/s）','绘制数据','View构建','操作','用户标识'],
			x: 'left'
		},
		dataZoom: [
			{
...
function show_fragmentDetail(fatherTable,fragmentInfo,pageLoadInfo){
	...
	var chart_option= {
		...
		legend: {
			data:['Activity生命周期函数','卡顿数据','流畅值（帧/s）','View构建','操作','用户标识'],
			x: 'left'
		},
		dataZoom: [
			{
...
function html_overViewTableDetail(fatherTable,data){
	...
	var html = ""
		...
		+'</div>'
        +'<div class="row" style="margin-left:10px;margin-right:10px;">'
		+'<h5 class="" style="color:#4876FF">2)页面绘制 ：</h5>'
		+'</div>'
        +'<div class="row" style="margin-left:10px;margin-right:10px;border:1px solid #aaa;">'
		+'	<div id="overViewTableDetail_Path_'+idName+'" class=""></div>'
		+'</div>';
...
function show_overViewTableDetail(fatherTable,data){
	...
	var idName = fatherTable+data.dataID;
	var pageLoadMore_Title_id = 'overViewTableDetail_Title_'+idName;
	var pageLoadMore_path_id = 'overViewTableDetail_Path_'+idName;
	var pageLoadMore_Chart_id = 'overViewTableDetail_Chart_'+idName;
	...
	//布局层次
    var path = data.viewDrawInfo.drawPath.split(';');
    var html = "";
    for(var i=0 ; i<path.length; i++) {
        html += "<p>" + path[i] + "</p>";
    }
    $("#"+pageLoadMore_path_id).html(html);
}
		
// -------------------------------------------------------------------------------------
// -----------------------------<用户标识页相关>--------------------------------------------
// -------------------------------------------------------------------------------------
function show_userTagTable(){
	$("#div_userTagTable").html(
			'<table id="table_userTagData" class="display" >'
				+'<thead>'
					+'<tr>'
						+'<th>方法名</th>'
						+'<th>耗时(ms)</th>'
		...
		var userTagData = getTableData_userTag();
		var div_table = $('#table_userTagData');
		if (div_table.hasClass('dataTable')) {
			var dttable = div_table.dataTable();
			dttable.fnClearTable(); //清空一下table
			dttable.fnDestroy(); //还原初始化了的datatable
		}
		var table_userTagData =div_table.DataTable( {
			data: userTagData,
			//每列所对应的数据字段：
			"columns": [
				{ "data": "methodName" },
		...
		$('#table_userTagData tbody').on( 'click', 'tr td.details-control', function () {
			if(isChildTableDetail){
				isChildTableDetail = false;
				return ;
			}
			var tr = $(this).closest('tr');
			var row = table_userTagData.row(tr);
			if ( row.child.isShown() ) {
				// This row is already open - close it
				row.child.hide();
				tr.removeClass('shown');
				...
				// Open this row
				row.child( html_userTagTableDetail('userTag_',row.data())).show();//绘制HTML
				show_userTagTableDetail('userTag_',row.data())//设置展示的数据
				tr.addClass('shown');
			}
		} );
}

function html_userTagTableDetail(fatherTable,data){
	var idName = fatherTable+data.dataID;
	var pageLoadMore_Title_id = 'userTagTableDetail_Title_'+idName;
	var pageLoadMore_Chart_id = 'userTagTableDetail_Chart_'+idName;

	var html = ""
		+'<div class="row" style="margin-left:10px;margin-right:10px;">'
		+'	<h5 class=""  style="color:#4876FF"><b id="userTagTableDetail_Title_'+idName+'">XXX页面：</b></h5>'
		+'</div>'
		+'<div class="row" style="margin-left:10px;margin-right:10px;">'
		+'	<h5 class="" style="color:#4876FF">1)方法运行详细图 ：</h5>'
		+'</div>'
		+'<div class="row" style="margin-left:10px;margin-right:10px;border:1px solid #aaa;">'
		+'	<div id="userTagTableDetail_Chart_'+idName+'" class="" style="height:350px" ></div>'
		+'</div>';

	return html;
}

function show_userTagTableDetail(fatherTable,data){
	var pageLoadInfo = data.pageLoadInfo;
	var idName = fatherTable+data.dataID;
	var pageLoadMore_Title_id = 'userTagTableDetail_Title_'+idName;
	var pageLoadMore_Chart_id = 'userTagTableDetail_Chart_'+idName;
	...
	//用户标识区间
	var startTime = getPageStartTime(pageLoadInfo);
    var charUserTag = new Array();
    var page = new Array();
    var start = data.start - startTime;
    var end = data.end - startTime;
    page.push({xAxis: start});
    page.push({xAxis: end});
    charUserTag.push(page);
	...
	var chart_option= {
		...
		tooltip: {
			...
			formatter: function(params){
				...
				tip = tip + getToolTip_ViewBuild(xAxisValue);
				tip = tip + getToolTip_Lifecycle(pageLoadInfo,xAxisValue);
				tip = tip + getToolTip_SingleUserTag(xAxisValue,data.start,data.end);
				return tip;
			}
		}
	};
...
// -----------------------------<内存泄漏页相关>--------------------------------------------
// -------------------------------------------------------------------------------------
function show_leakTable() {
	$("#div_LeakTable").html(
			'<table id="table_div_LeakTable" class="display" >'
				+'<thead>'
					+'<tr>'
						+'<th>时间</th>'
						+'<th>类名</th>'
    ...
    var div_table = $('#table_div_LeakTable');
    var tableLeakData = getTableData_leak();
    if (div_table.hasClass('dataTable')) {
    	var dttable = div_table.dataTable();
    	dttable.fnClearTable(); //清空一下table
    	dttable.fnDestroy(); //还原初始化了的datatable
    }
    var table_div_LeakTable =div_table.DataTable( {
    			data: tableLeakData,
    			//每列所对应的数据字段：
    			"columns": [
    				{ "data": "time" },
